...
### Create an offering (Teacher)
```bash
curl -X POST http://localhost:8080/v1/teacher/offerings \
...
### Add a session to an offering (Teacher)
```bash
curl -X POST http://localhost:8080/v1/teacher/offerings/<offeringId>/sessions \
  -H "Content-Type: application/json" \
  -H "X-User-Id: a0000000-0000-0000-0000-000000000001" \
  -d '{
    "startTime": "2026-06-01T09:00:00",
...
```bash
curl -X GET http://localhost:8080/v1/teacher/offerings \
  -H "X-User-Id: a0000000-0000-0000-0000-000000000001"
```

### List available offerings (Parent)
```bash
curl -X GET http://localhost:8080/v1/parent/offerings \
  -H "X-User-Id: b0000000-0000-0000-0000-000000000002"
```

### Book an offering (Parent)
```bash
curl -X POST http://localhost:8080/v1/parent/offerings/<offeringId>/book \
  -H "X-User-Id: b0000000-0000-0000-0000-000000000002"
```

### Get parent bookings
```bash
curl -X GET http://localhost:8080/v1/parent/bookings \
  -H "X-User-Id: b0000000-0000-0000-0000-000000000002"
```

### Postman Notes

Use the `X-User-Id` header for authenticated requests. Set `Content-Type: application/json` for POST requests and replace `<offeringId>` with the UUID returned from the offering creation response.
